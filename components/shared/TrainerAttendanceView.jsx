...
export default function TrainerAttendanceView({
  ...
  const [selectedTrainerId, setSelectedTrainerId] = useState(initialTrainerId || null);
  ...
  const fetchTrainers = useCallback(async () => {
    ...
    try {
      ...
      if (list.length > 0) {
        const requestedTrainerId = initialTrainerId;
  ...
  }, [initialTrainerId, selectedGym?.id, selectedTrainerId, authUser?.id]);
  ...
  if (roleLoading || loading) {
    return (
      <div className={showHeader ? "min-h-screen bg-gray-50 pb-24" : ""}>
        {showHeader && <Header title="Trainer Attendance" />}
  ...
  return (
    <div className={showHeader ? "min-h-screen bg-gray-50 pb-24" : ""}>
      ...
      <main className="px-4 py-4 space-y-4">
        {/* The Member/Trainer switch belongs to the page this is embedded in,
