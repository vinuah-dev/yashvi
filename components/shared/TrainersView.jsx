...
export default function TrainersView({ showHeader = true }) {
  ...
  const handleDeleteTrainer = async (trainer) => {
    ...
    try {
      ...
      // 0b) Remove the fingerprint from the gym's scanners. The F22 matches
      // locally and opens the gate before the server hears about it, so a
      // removed trainer would otherwise keep walking in forever. Failing this
      ...
      try {
        ...
        console.warn("Could not queue fingerprint removal:", bioErr);
      }
  ...
  return (
    <div className={showHeader ? "min-h-screen bg-gray-50 pb-24" : "pb-24"}>
      {showHeader && <Header title="Trainer Management" />}
